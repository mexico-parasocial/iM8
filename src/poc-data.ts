...
export function buildPersonas(handle: string): Persona[] {
  ...
  const secondHandle = generateAnonymousHandleFromText(handleRoot, 97)
  ...
  return [
    ...
    },
    {
      id: 'anon-isolated',
      name: secondHandle,
      handle: `@${secondHandle}.im8.local`,
      role: 'Independent anonymous',
      oneLine: 'A separate anonymous card for isolated contexts.',
      summary:
      ...
      signals: [
        {
          label: 'Visibility',
          value: 'Anonymous, unlinked to the first card',
          visibility: 'Public',
          action: 'Preview',
        },
        {
...
export function buildPublicPersona(handle: string, linkedProviders: SocialProvider[] = []): Persona {
  const cleanHandle = handle.trim().replace(/^@/, '') || 'public-card'
  const providerLabel = linkedProviders.length > 0
    ? linkedProviders.map((provider) => provider === 'bsky' ? 'Bluesky' : provider === 'x' ? 'X' : 'Instagram').join(', ')
    : 'm8-native'
  ...
  return {
    ...
    name: cleanHandle,
    handle: `@${cleanHandle}`,
    role: 'Optional public identity',
    oneLine: 'Created only after you choose to be discoverable.',
    summary:
      'This public card exists because you explicitly linked or created it. It can present selected proofs openly without merging the anonymous cards or exposing the private civic root.',
    kind: 'public',
    ...
    signals: [
      ...
      {
        label: 'Anonymous separation',
        value: 'Not linked to anonymous cards',
        visibility: 'Private',
        action: 'Review',
      },
    ],
...
export function buildCommunityAdmins(): CommunityAdmin[] {
  return [
    {
      did: 'did:plc:para-admin-01',
      displayName: 'Ana Admin',
      role: 'admin',
      status: 'active',
    },
    {
      did: 'did:plc:para-admin-02',
      displayName: 'Bruno Admin',
      role: 'admin',
      status: 'active',
    },
    {
      did: 'did:plc:para-admin-03',
...
export function buildPolicyChangeRequests(admins: CommunityAdmin[] = buildCommunityAdmins()): PolicyChangeRequest[] {
  return [
    {
      id: 'policy-change-public-figure-review',
      title: 'Update public figure review policy',
      summary:
        'Require community admin validation before changing durable PARA rules for public-figure verification and civic proof compatibility.',
      policyRecord: 'com.para.identity',
      proposedBy: 'PARA verifier',
      status: 'awaiting_admins',
