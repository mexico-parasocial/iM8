...
export function anonymousHandleSeed(input: string, offset = 0): number {
  return input.split('').reduce((seed, char, index) => {
    return seed + char.charCodeAt(0) * (index + 17)
  }, 137 + offset)
}

export function generateAnonymousHandleFromText(input: string, offset = 0): string {
  return generateAnonymousHandle(anonymousHandleSeed(input, offset))
}
