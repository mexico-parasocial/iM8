...
export async function identityRoutes(fastify: FastifyInstance) {
  ...
  // M8 stores only an opaque client-encrypted blob. Matrix keys never pass
  // through this service in plaintext.

  fastify.post('/identity/chat-key-backup', { preHandler: requireAuth }, async (request, reply) => {
    const parsed = chatKeyBackupSchema.safeParse(request.body)
    if (!parsed.success) {
    ...
    `).run(
      ...
      body.deviceId,
      body.ciphertext,
      JSON.stringify(body.encryptionMetadata),
      now,
      now,
    )
    ...
    `).run(
      request.sessionId!,
      'ChatKeyBackupUpserted',
      'chat_key_backup',
      request.sessionId!,
      JSON.stringify({ version: body.version, deviceId: body.deviceId }),
      now,
    )
  ...
  fastify.get('/identity/chat-key-backup', { preHandler: requireAuth }, async (request, reply) => {
    const db = getDb()
    ...
    if (!row) {
      return reply.status(404).send({ error: 'Chat key backup not found' })
    }
  ...
  fastify.delete('/identity/chat-key-backup', { preHandler: requireAuth }, async (request, reply) => {
    const db = getDb()
    const now = new Date().toISOString()
    const result = db.prepare('DELETE FROM chat_key_backups WHERE session_id = ?').run(request.sessionId!)
    ...
    `).run(
      request.sessionId!,
      'ChatKeyBackupDeleted',
      'chat_key_backup',
      request.sessionId!,
      JSON.stringify({ deleted: result.changes > 0 }),
      now,
    )

    return reply.send({ deleted: result.changes > 0, deletedAt: now })
  })
